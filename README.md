...
**DA Configuration:**
- `nodeRpc`: Ip address of your light/full node (ex: http://localhost:26658). Please follow [nubit's instruction](https://docs.nubit.org/nubit-da/run-a-node) for setting up a node.
- `authToken`: The authToken of your node, showing up when setting up node.
- `namespace`: Your designated namespace identifier.
- `fetchTimeout`: Time limit of fetching checkpoints(ex: 1m30s).
- `submitTimeout`: Time limit of submitting checkpoints.
